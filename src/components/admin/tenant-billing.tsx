    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          />
          <Value
            label={adminI18n.t("billing.billingHealth")}
            value={subscription.lastSyncErrorCode ?? subscription.status}
          />
          <Value
          ...
          <Value
            ...
            value={
              subscription.billingPeriod
                ? adminI18n.formatDateTime(
                    subscription.billingPeriod.periodStart,
                  )
          ...
          <Value
            ...
            value={
              subscription.billingPeriod
                ? adminI18n.formatDateTime(subscription.billingPeriod.periodEnd)
                : adminI18n.t("empty.notRecorded")
            }
          />
          <Value
        ...
        <p className="mt-5 border-t border-gray-100 pt-4 text-sm text-gray-600">
          {billing.overrideState === "ACTIVE"
            ? adminI18n.t("billing.overrideActiveWarning")
            : adminI18n.t("billing.defaultPolicy")}
        </p>
      </section>
    </div>
  );
    ...
    <section className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
      ...
      <details className="mt-6 border-t border-gray-100 pt-4">
        <summary className="cursor-pointer text-sm font-semibold text-gray-700">
          {adminI18n.t("billing.advancedLimits")}
        </summary>
...
function Shopify({ billing }: { billing: TenantBilling }) {
  ...
  return (
    <section className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
      <h2 className="mb-4 text-lg font-semibold text-gray-950">
        {adminI18n.t("billing.tab.shopify")}
      </h2>
      <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Value
        ...
        />
        <Value
          label={adminI18n.t("billing.providerStatus")}
          value={subscription.lastSyncErrorCode ?? subscription.status}
        />
      </dl>
      {billing.discrepancy ? (
        <dl className="mt-6 grid gap-4 border-t border-gray-100 pt-4 sm:grid-cols-2">
          <Value
            label={adminI18n.t("billing.modaQuantity")}
            value={billing.discrepancy.modaQuantity}
          />
          <Value
            label={adminI18n.t("billing.shopifyQuantity")}
            value={billing.discrepancy.shopifyQuantity}
          />
        </dl>
      ) : (
        <p className="mt-6 border-t border-gray-100 pt-4 text-sm text-gray-600">
          {adminI18n.t("billing.reconciliationUnavailableShort")}
        </p>
      )}
    </section>
  );
...
function Activity({
  ...
  packs: PageResult<RecoveryCreditPurchaseItem>;
  events: PageResult<BillingLedgerItem>;
  params: Record<string, string>;
}) {
  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-200 px-5 py-4">
          <h2 className="text-lg font-semibold text-gray-950">
            {adminI18n.t("billing.tab.recoveryPacks")}
          </h2>
        </div>
        {packs.items.length ? (
              ...
              <tbody className="divide-y divide-gray-200">
                {packs.items.map((purchase) => (
                  <tr key={purchase.id}>
                    ...
                    <td className="px-5 py-3 text-sm text-gray-600">
                      {adminI18n.formatNumber(purchase.creditsGranted)}
                    </td>
                    <td className="px-5 py-3 text-sm text-gray-600">
                    ...
                    <td className="px-5 py-3 text-sm">
                      <Link
                        ...
                        href={withParamUpdates("/", params, {
                          billingView: "activity",
                          purchaseId: purchase.id,
                        })}
                      >
                        {adminI18n.t("billing.details")}
                      </Link>
                    </td>
                  </tr>
                ))}
          ...
          <p className="px-5 py-6 text-sm text-gray-500">
            {adminI18n.t("billing.noRecoveryPacks")}
          </p>
        )}
        <Pagination
          pathname="/"
          ...
          totalItems={packs.totalItems}
          pageParam="packPage"
          countKey="pagination.items"
          resetParams={["packPage", "purchaseId"]}
        />
      </section>
      <section className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-200 px-5 py-4">
          <h2 className="text-lg font-semibold text-gray-950">
            {adminI18n.t("billing.tab.appEvents")}
          </h2>
        </div>
        {events.items.length ? (
          <div className="overflow-x-auto">
        ...
        <Pagination
          ...
          countKey="pagination.items"
          resetParams={["billingPage", "eventId"]}
        />
      </section>
    </div>
  );
...
export function TenantBillingView({
  ...
  packs: PageResult<RecoveryCreditPurchaseItem> | null;
  events: PageResult<BillingLedgerItem> | null;
  ...
  return (
    <div>
      ...
      {billingView === "activity" && packs && events ? (
        <Activity packs={packs} events={events} params={params} />
      ) : null}
      {selectedPurchase ? (
        <RecoveryCreditPurchaseDrawer
