...
import { PromotionTranslationWorkbook } from "@/components/admin/promotion-translation-workbook";
import { buildPromotionTranslationTemplate } from "@/lib/admin/promotion-translations";
...
export function PromotionCampaignForm({
  ...
  const englishTitle = campaign?.englishMerchantTitle ?? "";
  const englishDescription = campaign?.englishMerchantDescription ?? "";
  return (
    <>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="text-sm font-medium text-gray-700">
            ...
            <select
              ...
              value={scope}
              onChange={(event) =>
                setScope(event.target.value as PromotionScope)
              }
              disabled={!editable}
            >
      ...
      {campaign?.status === "DRAFT" ? (
        <PromotionTranslationWorkbook
          ...
          completeCount={campaign.translationCount}
          template={buildPromotionTranslationTemplate({
            campaignId: campaign.id,
            campaignInternalName: campaign.name,
            sourceMerchantTitle: englishTitle,
            sourceMerchantDescription: englishDescription,
...
export function ActivatePromotionCampaignForm({
  ...
  return campaign.status === "DRAFT" && campaign.translationCount === 20 ? (
    <form action={mutatePromotionCampaignAction}>
      <input type="hidden" name="intent" value="activate" />
      <input type="hidden" name="id" value={campaign.id} />
      <button
        type="submit"
        className="rounded-md border border-green-700 px-3 py-1.5 text-xs font-semibold text-green-800 hover:bg-green-50"
      >
        Activate
      </button>
    </form>
...
export function ClosePromotionCampaignForm({
  ...
  return (
    <form action={mutatePromotionCampaignAction}>
      ...
      <button
        type="submit"
        className="rounded-md border border-red-300 px-3 py-1.5 text-xs font-semibold text-red-800 hover:bg-red-50"
      >
        Close
      </button>
    </form>
...
export function ReopenPromotionCampaignForm({
  campaign,
}: {
  campaign: PromotionCampaignRow;
}) {
    ...
    >
      ...
      <label className="text-xs font-medium text-gray-600">
        ...
        <input
          ...
          name="expiresAt"
          type="datetime-local"
          min={localDateTime(campaign.startsAt)}
          required
        />
      </label>
      <button
